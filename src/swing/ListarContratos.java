package swing;

import java.awt.Color;
import java.awt.Component;
import java.util.Vector;

import javax.swing.WindowConstants;
import javax.swing.table.DefaultTableCellRenderer;
import javax.swing.table.DefaultTableModel;
import javax.swing.table.TableCellRenderer;
import javax.swing.table.TableColumnModel;
import javax.swing.table.TableModel;
import javax.swing.BorderFactory;
import javax.swing.ImageIcon;
import javax.swing.JLabel;
import javax.swing.JScrollPane;
import javax.swing.JTable;

import controlador.SistemaCocheras;
import vista.ContratoView;

public class ListarContratos extends javax.swing.JFrame {

	private static final long serialVersionUID = 1L;
	private JTable contratos;
	private TableModel jTable1Model;
	
	private SistemaCocheras sistemaCocheras;
	private String dniCliente;
	private boolean vigentes;
	
	public ListarContratos(SistemaCocheras s, String d, Boolean v) {
		super();
		sistemaCocheras = s;
		dniCliente = d;
		vigentes = v;
		initGUI();		
	}
	
	private void initGUI() {
		try {
			setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
			//getContentPane().setLayout(null);
			{				
				Vector<ContratoView> contratoView = sistemaCocheras.listarContratos(dniCliente, vigentes);
				Vector<Object> vectorOfVectors = new Vector<Object>();
				for (int i= 0; i < contratoView.size(); i++)
				{
					vectorOfVectors.add((contratoView.elementAt(i)).toVector());
				}
				
				Vector<String> columnas = new Vector<String>();
				columnas.add("N∫ contrato");
				columnas.add("Fecha");				
				columnas.add("Patente");
				columnas.add("N∫ cochera");
				columnas.add("Abono");
				columnas.add("Estado");
				
				jTable1Model = new DefaultTableModel(vectorOfVectors, columnas){
					
					private static final long serialVersionUID = 1L;

					public boolean isCellEditable(int row, int column){
						return false;
					}
				};
				contratos = new JTable();				
				contratos.setModel(jTable1Model);				
				
				DefaultTableCellRenderer centerRenderer = new DefaultTableCellRenderer();
				centerRenderer.setHorizontalAlignment(JLabel.CENTER);
				contratos.getColumnModel().getColumn(0).setCellRenderer(centerRenderer);
				contratos.getColumnModel().getColumn(1).setCellRenderer(centerRenderer);
				contratos.getColumnModel().getColumn(3).setCellRenderer(centerRenderer);
				contratos.getColumnModel().getColumn(5).setCellRenderer(centerRenderer);
				
				//Tama√±o de las columnas
				resizeColumnWidth(contratos);
				
				//Borde Negro
				contratos.setBorder(BorderFactory.createLineBorder(Color.BLACK,1));
				
				JScrollPane scrollPane = new JScrollPane(contratos);
				
				getContentPane().add(scrollPane);

			}
			ImageIcon img = new ImageIcon("src/swing/contract.png");
			setIconImage(img.getImage());
			pack();
			setSize(750, 450);
			setTitle("Contratos del Cliente: " + dniCliente);
			setLocationRelativeTo(null);
			setResizable(false);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	
	//Tama√±o de las columnas
	public void resizeColumnWidth(JTable table) {
	    final TableColumnModel columnModel = table.getColumnModel();
	    for (int column = 0; column < table.getColumnCount(); column++) {
	        int width = 50; // Min width
	        for (int row = 0; row < table.getRowCount(); row++) {
	            TableCellRenderer renderer = table.getCellRenderer(row, column);
	            Component comp = table.prepareRenderer(renderer, row, column);
	            width = Math.max(comp.getPreferredSize().width +1 , width);
	        }	        
	        columnModel.getColumn(column).setPreferredWidth(width);
	    }
	}
	
}
